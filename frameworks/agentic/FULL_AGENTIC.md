...
- **Declare and enforce fan-out budgets in skill manifests.** Open-loop composition is its own supply-chain risk. Without declared fan-out caps and cooldowns, a discovery skill can fan into N enrichment calls → M drafting attempts → unbounded retries, with no individual skill misbehaving. Skill manifests must declare `max_fan_out` and `cooldown_seconds`; sub-skills that spawn further sub-skills transitively inherit the parent budget, not a fresh one.
- **Treat skill discovery as a supply-chain event, not browsing.** AgentBaiting and FakeGit-style attacks (e.g., 800+ fake AI skills in public GitHub repos) demonstrate that an agent can be led to summarize and relay attacker-controlled install instructions *before* any human sees the link. Lookup, not just install, is part of the attack surface. Use reviewed internal catalogs; add provenance checks before surfacing install instructions from open registries or lookalike repos.
- **Scheduler fast-track flags are an incentive-misalignment vector.** Tool providers embed "fast-track" metadata that schedulers honor without reviewing what safety checks get skipped. A scheduler rewarded for throughput + a provider rewarded for usage = a cooperative path around guardrails. Audit trails must record *why* an execution path was chosen, not just what ran.
...
- **Make decision transparency explicit for unattended workflows.** Filtering, timing, scope expansion, tone softening, and omission are operational decisions, not invisible glue. For cron jobs, inbox triage, and autonomous assistants, record summary counts of what was filtered, deferred, softened, or auto-recovered so humans can inspect the curation layer instead of only the polished outcome.
- **Delete incidental behavioral predictions from memory.** Patterns about approval timing, fatigue, responsiveness windows, or emotional receptivity are not harmless optimization residue; they are persuasion material if memory leaks. Keep only behavior notes that are clearly necessary for safe operation, and schedule periodic pruning of the rest.
- **Treat identity-file edits as policy changes.** If an agent can edit SOUL/AGENTS/IDENTITY files, review the diff the way you would review a guardrail change: small wording changes around autonomy, compliance, interruption, or tone can accumulate into materially different behavior.
