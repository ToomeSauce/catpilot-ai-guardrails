...
- **Unattended write policy:** ❌ NEVER treat "the human can revert it later" as sufficient approval for sleep-time or cron-driven writes to shell aliases, dashboards, views, workflow config, or other ambient tooling. ✅ Always restrict unattended jobs to pre-approved write classes and emit a morning-visible trace of what changed.
...
- **Cron trace model:** ✅ Keep action, rejection, and handoff traces for unattended runs so humans can reconstruct not just what changed, but what was considered, blocked, or escalated.
...
- **Network egress as memory risk:** Treat telemetry, crash reporting, query parameters, and referer leakage as part of memory security. ✅ Scrub or aggregate workspace-derived payloads before outbound requests. ❌ NEVER assume non-secret memory fragments are harmless once third-party egress exists.
...
- **Skill instruction/data split:** Treat registry metadata, install manifests, README prose, and fetched tool output as untrusted data until a trusted control path decides otherwise. ❌ NEVER let a skill smuggle executable policy through descriptive fields like "telemetry," "compliance," or "internal config."
- **Three-log discipline for unattended work:** Keep action, rejection, and handoff traces together. ❌ NEVER ship a cron/nightly system that only records successes; invisible filtering and rejected actions are part of the reliability story.
